...
class Scorer:
    ...
    def get_protein_scores(
        ...
        s_matrix = np.zeros(
            (len(query_system.interacting_protein_chains), len(target_protein_chains))
        )
        for i, q_instance_chain in enumerate(query_system.interacting_protein_chains):
        ...
        while np.any(s_matrix):
            ...
            q_instance_chain, t_instance_chain = (
                query_system.interacting_protein_chains[q_idx],
    ...
    def get_scores_holo(
        ...
        query_system_length = sum(
            ...
            .length
            for q_instance_chain in query_system.interacting_protein_chains
        )
        query_instance_chains = [
            chain.split(".")[1] for chain in query_system.interacting_protein_chains
        ]
        for target_entry in query_entry_alignments.index.get_level_values(
            ...
            for target_system_id in self.entries[target_entry].systems:
                ...
                if target_system_id == query_system.id or all(
                    ...
                    for target_instance_chain in target_system.interacting_protein_chains
                ):
                ...
                (
                    ...
                    query_system,
                    target_system.interacting_protein_chains,
                    query_system_length,
                )
    ...
    def get_scores_apo_pred(
        ...
        q_chain = query_system.interacting_protein_chains[0].split(".")[1]
        query_system_length = sum(
            self.entries[query_system.pdb_id]
            .chains[q_instance_chain.split(".")[1]]
            .length
