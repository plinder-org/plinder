...
def plan_protein_scoring(
    ...
    *,
    ...
    chains = chains[
        ...
        & chains["entry_pdb_id"].isin(query_ids)
    ]
    counts = chains.groupby("entry_pdb_id").size().to_dict()
...
def _load_plan(data_dir: Path, *, recheck_source: bool = False) -> dict[str, Any]:
    ...
    if recheck_source:
        source = Path(str(plan["entry_chains"]["path"]))
        if _source_signature(source) != plan["entry_chains"]:
            raise ValueError(
                "entry chain index changed after protein scoring was planned"
            )
    return plan
...
def plan_score_batches(
    ...
    *,
    ...
    if min(batch_size, threads, max_protein_chains, max_ligand_chains) < 1:
    ...
    hit_work_sql = dedent(
        ...
        LEFT JOIN ligand_counts AS target
    ...
    work = connection.sql(
        dedent(
            f"""
            WITH eligible_annotation AS (
                SELECT *
                FROM read_parquet('{annotation.as_posix()}')
                WHERE system_type = 'holo'
                  AND system_num_protein_chains <= {max_protein_chains}
                  AND system_num_ligand_chains <= {max_ligand_chains}
            ), ligand_counts AS (
                SELECT
                    entry_pdb_id,
                    count(*) FILTER (
                    ...
                    count(DISTINCT ligand_asym_id) FILTER (
                        WHERE coalesce(ligand_is_proper, false)
                          AND coalesce(ligand_is_3d_score_able, false)
                    )::DOUBLE AS scoreable_canonical_ligands
                FROM eligible_annotation
                GROUP BY entry_pdb_id
            ), eligible_queries AS (
            ...
            LEFT JOIN ligand_counts AS query_ligands
    ...
    plan.update(
        {
            ...
            "score_max_protein_chains": max_protein_chains,
            "score_max_ligand_chains": max_ligand_chains,
...
def plan_clustering(
    ...
    *,
    ...
    """Report exact array bounds for component reduction and communities."""
    ...
    source_batches = tasks.scatter_component_reduction_sources(
        data_dir=data_dir,
    ...
    return {
        ...
        "component_reduction_batch_count": math.ceil(source_count / source_batch_size),
...
def summarize_clustering_artifacts(
    ...
    *,
    ...
    combinations = [
        (metric, threshold, cluster, directed)
    ...
    expected_paths = {
        ...
        for metric, threshold, cluster, directed in combinations
    }
    rows: list[dict[str, Any]] = []
    issues: list[str] = []
    started = perf_counter()
    for index, (metric, threshold, cluster, directed) in enumerate(
        ...
        if index % 10 == 0 or index == len(combinations):
            ...
            LOG.info(
                ...
                f"validated={index}/{len(combinations)} rate={rate:.2f}/s "
                f"eta_seconds={(len(combinations) - index) / rate:.1f}"
            )

    stats = pd.DataFrame(rows)
    if len(stats) == len(combinations):
        for metric in selected_metrics:
            ...
            for threshold in selected_thresholds:
                ...
                counts = {
                    (str(row.cluster), bool(row.directed)): int(row.cluster_count)
                    for row in at_threshold.itertuples(index=False)
                }
                weak = counts[("components", False)]
                strong = counts[("components", True)]
                communities = counts[("communities", False)]
                if strong < weak:
                    issues.append(
                        f"{metric} threshold={threshold} has fewer strong than weak "
                        f"components: {strong} < {weak}"
                    )
                if communities < weak:
                    issues.append(
                        f"{metric} threshold={threshold} has fewer communities than "
                        f"weak components: {communities} < {weak}"
                    )
    ...
    report = {
        ...
        "expected_artifact_count": len(combinations),
...
def _parser() -> argparse.ArgumentParser:
    ...
    score_plan.add_argument("--max-protein-chains", type=int, default=5)
    score_plan.add_argument("--max-ligand-chains", type=int, default=5)
    ...
    cluster_plan.add_argument("--source-batch-size", type=int, default=1)
    ...
    for name in [
        ...
        if name in {"component-reductions", "communities"}:
            _add_cluster_arguments(command)
        if name == "export-sucos-shards":
...
def main() -> None:
    ...
    if args.command == "plan":
        ...
        result = plan_score_batches(
            ...
            max_ligand_chains=args.max_ligand_chains,
        )
    ...
    elif args.command == "export-sucos-shards":
        ...
        cfg = _scoring_config(
            data_dir,
            int(plan["max_seqs"]),
            max_protein_chains=int(plan.get("score_max_protein_chains", 5)),
            max_ligand_chains=int(plan.get("score_max_ligand_chains", 5)),
        )
        scratch_dir = args.scratch_dir.resolve()
        scratch_dir.mkdir(exist_ok=True, parents=True)
