...
class Ligand(DocBaseModel):
    ...
    smiles: str = Field(
        default_factory=str,
        description="Ligand SMILES from CCD lookup (user-supplied SMILES wins for custom CIFs), or derived from resolved 3D if not in the dictionary",
    )
    ...
    failed_interaction_types: list[str] = Field(
        default_factory=list,
        description="__Interaction types whose peppr detector raised for this "
    ...
    def set_rdkit(self) -> None:
        ...
        try:
            ...
            if descriptors is not None:
                ...
                # num_heavy_atoms is set at construction (with the resolved /
                # unresolved counts), not here.
